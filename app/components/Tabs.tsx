  ...
  const headers = props.tabBodies.map((tabBody, i) => {
    return (
      <div className="st-details-header me-2 cursor-pointer" key={i + tabBody.title} 
        ...
        <h4 className={classNames(activeTab === i ? 'text-cyan-900 hover:text-cyan-600 dark:text-cyan-500 dark:hover:text-cyan-500 border-b border-cyan-600 dark:border-cyan-500' : 'hover:text-gray-600 hover:border-b hover:border-gray-300 dark:hover:text-gray-300', 'text-sm inline-block pb-1',)}>
          {tabBody.title}
        </h4>
      </div>
    )
